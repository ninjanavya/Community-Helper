    ...
    if USE_GEMINI and genai_client:
        try:
            ...
            if pil_image:
                ...
                logger.info("[AUDIT] Uploaded image IS being sent to Gemini model generate_content.")
            else:
                logger.info("[AUDIT] No image provided. Sending description-only text prompt to Gemini.")
            ...
            logger.info(f"[AUDIT] Raw Gemini response text: {response_text}")
            ...
            logger.info("[AUDIT] CIE Incident Intelligence Report generated by Gemini successfully.")
            return res_dict
                
        except Exception as e:
            logger.error(f"[AUDIT] Gemini API call or validation failed: {e}. Falling back to mock generator.", exc_info=True)
            ...
            logger.info("[AUDIT] CIE Incident Intelligence Report generated by Fallback (Gemini call failed).")
            return res_dict
    else:
        logger.info("[AUDIT] Gemini client not configured. Generating mock response.")
        res_dict = _generate_mock_iir(desc_clean)
        res_dict["iir_status"] = "fallback"
        res_dict["iir_message"] = "AI analysis temporarily unavailable. A basic report has been generated."
        logger.info("[AUDIT] CIE Incident Intelligence Report generated by Fallback (Gemini not configured).")
        return res_dict
...
def validate_incident_image(image: Any, description: str) -> Dict[str, Any]:
    ...
    if USE_GEMINI and genai_client:
        try:
            ...
            logger.info("[AUDIT] Uploaded image IS being sent to Gemini model generate_content for image validation.")
            ...
            response_text = response.text
            logger.info(f"[AUDIT] Raw Gemini image validation response text: {response_text}")
            if not response_text:
            ...
            if hasattr(validated, "model_dump"):
                ...
                res_dict = validated.dict()
            logger.info(f"[AUDIT] AVE returned is_valid = {res_dict.get('is_valid')} (via Gemini)")
            return res_dict
                
        except Exception as e:
            logger.error(f"[AUDIT] Gemini API validation call failed: {e}. Falling back to default success.", exc_info=True)
            return {
                "is_valid": True,
        ...
        logger.info("[AUDIT] Gemini client not configured. Skipping image validation.")
        desc_lower = desc_clean.lower()
        if "selfie" in desc_lower or "cat" in desc_lower or "dog" in desc_lower or "person" in desc_lower:
            logger.info("[AUDIT] AVE returned is_valid = False (via Local Rule)")
            return {
                "is_valid": False,
        ...
        logger.info("[AUDIT] AVE returned is_valid = True (via Local Rule)")
        return {
            "is_valid": True,
